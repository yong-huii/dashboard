...
export function forceDirectedNetwork(
  ...
  const { fontSize = 10, colors, wrapColors = true } = options || {};
  ...
  networkSeries.fontSize = fontSize ?? 10;
  networkSeries.linkWithStrength = 0;

  // 최상위 노드(label)만 14px로 설정
  networkSeries.nodes.template.adapter.add("fontSize", (fontSize, target) => {
    // 루트 노드(부모가 없는 노드)만 14px 적용
    if (!target.dataItem.parent) return 16;
    return fontSize;
  });
  ...
  nodeTemplate.tooltip.label.fontSize = 10;
