...
export default function DataInfoTable({ data, isLoading }: Props) {
  ...
  return (
    <div
      className="row-span-3 hidden flex-col overflow-hidden rounded-lg bg-white px-4 pb-4 shadow-md lg:flex"
      ref={wrapRef}
    >
        ...
        <div className="border-x border-t border-[#F0F0F0]">
          <Table<RowItem>
            ...
            scroll={{
              x: "max-content",
              y:
                (wrapRef.current?.clientHeight || 0) < 240
                  ? (wrapRef.current?.clientHeight || 0) - 61
                  : undefined,
            }}
            showHeader={false}
            tableLayout="fixed"
